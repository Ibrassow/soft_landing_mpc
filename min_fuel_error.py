...
q = np.array([0,0]) 
q = np.array([4.55224032e+02, -2.90097058e+02])
...
## TODO UPDATE FROM MIN ERROR LANDING
constraints += [cp.norm2(E @ X[:3,N-1] - q) <= cp.norm2(np.array([0,0]) - q)]
...
cp.Problem(objective, constraints).solve(solver='MOSEK', verbose=True)
...
ax.set_title("Minimum fuel problem")"""
